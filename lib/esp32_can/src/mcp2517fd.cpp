bool debuggingMode; // TODO reeeeee
#include "Arduino.h"
#include <SPI.h>
#include "mcp2517fd.h"
...
void task_ResetWatcher(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (ctrlVal == 7)
        {
            ...
            if (mcpCan->debuggingMode)
                Serial.println("!!!RESTRICTED MODE!!!");
        }
        ...
        if (mcpCan->needMCPReset)
        {
            ...
            if (mcpCan->debuggingMode)
                Serial.println("Reset 2517FD hardware");
...
void MCP2517FD::resetHardware()
{
    ...
    for (idx = 0; idx < 8; idx++)
        ctrl[idx] = Read(ADDR_CiFLTCON + 4 * idx);

    if (inFDMode)
        initFD(savedNominalBaud, savedDataBaud);
    else
        init(savedNominalBaud);
    ...
    for (idx = 0; idx < 8; idx++)
        Write(ADDR_CiFLTCON + 4 * idx, ctrl[idx]);
}
...
void MCP2517FD::sendCallback(CAN_FRAME_FD *frame)
{
    ...
    if (frame->fid & 0x80000000ul) // object callback
    {
        ...
        if (isFD)
            thisListener->gotFrameFD(frame, mb);
        else
...
void MCP2517FD::initializeResources()
{
    ...
    if (debuggingMode)
        Serial.println("initializeResources()");
    ...
    if (debuggingMode)
        Serial.println("Initialized hardware pins");
    ...
    if (debuggingMode)
        Serial.println("Initialized queues");
    ...
    if (debuggingMode)
        Serial.println("Done with resource init");
...
void MCP2517FD::initSPI()
{
    ...
    if (debuggingMode)
        Serial.println("MCP2517FD SPI Inited");
}
...
bool MCP2517FD::_init(uint32_t CAN_Bus_Speed, uint8_t Freq, uint8_t SJW, bool autoBaud)
{
    ...
    {
        if (debuggingMode)
            Serial.println("Could not enter configuration mode");
        return false;
    }
    ...
    */
    ...
    {
        if (debuggingMode)
            Serial.println("Nominal TSeg1 outside of limits. Invalid baud rate!");
        return false;
    }
    if (tseg2 > 128 || tseg2 < 1)
    {
        if (debuggingMode)
            Serial.println("Nominal TSeg2 outside of limits. Invalid baud rate!");
        return false;
    }
    ...
    {
        if (debuggingMode)
            Serial.println("MCP2517 Init Success");
        errorFlags = 0;
        cachedDiag1 = 0;
        Write(ADDR_CiBDIAG0, 0); // clear debug flags too.
        Write(ADDR_CiBDIAG1, 0);
...
bool MCP2517FD::_initFD(uint32_t nominalSpeed, uint32_t dataSpeed, uint8_t freq, uint8_t sjw, bool autoBaud)
{
    ...
    if (debuggingMode)
        Serial.println("_initFD()");

    if (!initializedResources)
        initializeResources();

    if (nominalSpeed < 125000)
        return 0; // won't work, die - Keep in mind that the FD spec doesn't want less than 500k here though
    if (dataSpeed < 1000000ul)
        return 0; // also won't work.
    ...
    */
    ...
    {
        if (debuggingMode)
            Serial.println("Nominal TSeg1 outside of limits. Invalid baud rates!");
        return false;
    }
    if (tseg2 > 128 || tseg2 < 1)
    {
        if (debuggingMode)
            Serial.println("Nominal TSeg2 outside of limits. Invalid baud rates!");
        return false;
    }
    ...
    if (debuggingMode)
        Serial.printf("Nomimal Settings:  TSEG1: %u   TSEG2: %u\n", tseg1, tseg2);
    ...
    {
        if (debuggingMode)
            Serial.println("Data TSeg1 outside of limits. Invalid baud rates!");
        return false;
    }
    if (tseg2 > 16 || tseg2 < 1)
    {
        if (debuggingMode)
            Serial.println("Data TSeg2 outside of limits. Invalid baud rates!");
        return false;
    }
    ...
    if (debuggingMode)
        Serial.printf("Data Settings:  TSEG1: %u   TSEG2: %u\n", tseg1, tseg2);
    ...
    {
        if (debuggingMode)
            Serial.println("MCP2517 InitFD Success");
        errorFlags = 0;
        rxFault = false;
        txFault = false;
        faulted = false;
...
{
    if (!rxQueue)
        return 0; // why would this happen though?!
...
int MCP2517FD::_setFilter(uint32_t id, uint32_t mask, bool extended)
{
    ...
    if (debuggingMode)
        Serial.println("Err: No filter set!");
    return -1;
}
...
int MCP2517FD::_setFilterSpecific(uint8_t mailbox, uint32_t id, uint32_t mask, bool extended)
{
    ...
    if (mailbox > 31)
        return 0; // past end of valid mailbox #said
    ...
    if (extended)
        packedID |= 1 << 30; // only allow extended frames to match
...
uint32_t MCP2517FD::ReadFrameBuffer(uint16_t address, CAN_FRAME_FD &message)
{
    ...
    */
    ...
    if (message.extended)
        message.id = unpackExtValue(ptr_buff[0] & 0x1FFFFFFFull);
    else
        message.id = ptr_buff[0] & 0x7FF;
    message.fid = 0;
    message.priority = 0;
    ...
    for (int j = 0; j < copyWords; j++)
        message.data.uint32[j] = ptr_buff[3 + j];
...
void MCP2517FD::handleTXFifoISR(int fifo)
{
    ...
    if ((status & 0x20) == 0x20)
        needTXFIFOReset = true; // if the queue registered a fault then set flag to have it reset
    ...
    while ((status & 1) && (uxQueueMessagesWaiting(txQueue) > 0)) // while fifo is not full and we have messages waiting
    {
        ...
        if (inFDMode)
        {
            ...
            if (xQueueReceive(txQueue, &frameFD, 0) != pdTRUE)
                return; // abort if we can't load a frame from the queue!
        }
        else
        {
            // if (debuggingMode) Serial.write('S');
            if (xQueueReceive(txQueue, &frame, 0) != pdTRUE)
                return;
            // hardware loading below always uses the same buffer save whether CAN or CANFD
            if (!canToFD(frame, frameFD))
                return;
        }
